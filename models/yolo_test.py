...
class Model(nn.Module):
    ...
    def forward(self, x, x2, augment=False, profile=False): # augment是数据增强，profile是计算时间和FLOPS
        if augment:
    ...
    def forward_once(self, x, x2, profile=False):
        ...
        for m in self.model:
            ...
            if m.f != -1:  # if not from previous layer
                if m.f != -4:
                    # print(m)
                    # 如果m.f是整数，x就等于y[m.f]，否则x就是一个列表，x=[y[m.f[0]], y[m.f[1]]]
            ...
            #这里就负责调用每一层的forward函数
            if m.f == -4:
                x = m(x2)
            else:
                x = m(x)  # run
            y.append(x if m.i in self.save else None)  # save output 保存需要的输出
            # print(len(y))
            i+=1
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head']):  # from, number, module, args
        m = eval(m) if isinstance(m, str) else m  # eval strings m是模块类别，如Conv、BottleneckCSP等 eval将字符串转换为对应的类
        for j, a in enumerate(args):#j是args的索引，a是args的值
            try:
                args[j] = eval(a) if isinstance(a, str) else a  # eval strings eval将字符串转换为对应的值
            except:
                pass

        n = max(round(n * gd), 1) if n > 1 else n  # depth gain 调整模块重复次数
        if m in [Conv, GhostConv, Bottleneck, GhostBottleneck, SPP, DWConv, MixConv2d, Focus, CrossConv, BottleneckCSP,
                 C3, C3TR]:

            if m is Focus:
                c1, c2 = 3, args[0]  # 输入输出通道数
                # print("focus c2", c2)
                if c2 != no:  # if not output 如果不是检测头（检测头需要输出255个通道）
                    c2 = make_divisible(c2 * gw, 8) #调整输出通道数，并令其为8的倍数
                args = [c1, c2, *args[1:]] # 拼接输入输出通道数与其他参数 对应torch.nn.Conv2d(in_channels, out_channels, kernel_size, stride=1, padding=0, ...)
            else:
                c1, c2 = ch[f], args[0]
        ...
        m_ = nn.Sequential(*[m(*args) for _ in range(n)]) if n > 1 else m(*args)  # module 创建实例化对象，此时m_是具体的nn.Module对象
        t = str(m)[8:-2].replace('__main__.', '')  # module type
        np = sum([x.numel() for x in m_.parameters()])  # number params
        m_.i, m_.f, m_.type, m_.np = i, f, t, np  # attach index, 'from' index, type, number params
        logger.info('%3s%18s%3s%10.0f  %-40s%-30s' % (i, f, n, np, t, args))  # print
        save.extend(x % i for x in ([f] if isinstance(f, int) else f) if x != -1)  # append to savelist 保存需要的输出
        layers.append(m_) # 把构建好的层加入layers列表
        if i == 0:
            ch = []
        # if i == 4:
        #     ch = []
        ch.append(c2)  # 更新ch列表，把当前层的输出通道数加入ch列表 所以每个循环里的ch[-1]都是上一层的输出通道数
    # print(layers)
